...
|Gut geeignet zum Nachbilden hierarchischer Informationsflussbeschraenktungen, haeufig in Unternehmen, Behoerden|Sanitizing-Konzept: Zurueckstufen von Labelings|
|varianten der MLS-Policy in der Praxis verbreitet; u.a. integriert in Standardsoftware u.a. in SE-Linux|| 

## 8.3 Konzepte zur Implementierung des Rechtemanagements

**Basis:** Varianten einer Zugriffsmatrix
   - ZM i.d.R sparse
   - Implementierung als Tabelle ist ineffizient
  
**Klassische Konzepte in Betriebssystemen**
  - Zugriffskontrollisten, ACL
  - Capability-Listen

### 8.3.1 *A*ccess-*C*ontrol-*L*ist *ACL*
   - Rechte an Objekten
   - Spaltenweisen Realisierung der Zugriffsmatrix

|+|-|
|---|---|
|Einfache Implementierung, z.B.: direkt durch OS (i-Node unter Linux)|Fehleranfaelliges Rechtemanagement aus Subjekt-Sicht|
|Einfaches Enforcement/Kontrolle||
|Rechtemanagement aus Objekt-Sicht ist einfach||

### 8.3.2 Capability-Konzept
**Capability (Ticket, Token, Handle):**
   - Datenstruktur bestehend aus Objekt-UID und Rechte-Bits
   - Besitz der Capability berechtigt zur Rechte-Wahrnehmung
Zeilenweise realisierung der Zugriffs-Matrix

**Management von Capabilities/Token:**
Oft vorkommende Strukturen:
  1. Erstellt, verwaltet, geschuetzt als Datenstruktur des OS
  2. Erstellt als Access Token durch Autorisierungs-Server und Verteilung an Client-Rechner: u.a. OAuth-Dienst im Web-Umfeld

|+|-|
|---|---|
|SUbjekt-bezogene Rechteverwaltung ist einfach|Rechteruecknahme aus Objektsicht schwierig: Bereinigung aller offenen Capabilitylisten ist erforderlich|
|Ticketweitergabe/Delegation moeglich|Stehlen oder kopieren von Tickets ist moeglich, falls kein Schutz vorhanden ist. Capability besitzt keine Subjektbindung|

## 8.4 Zugriffskontrolle
   - i.d.R. Aufteilung in Berechtigungs- und Zulaessigkeitskontrolle
**Berechtigungskontrolle** *P*olicy *D*ecision *P*oint
   - Pruefung beim erstmaligen Zugriff auf ein Objekt
   - Ausstellung einer Bescheinigung
**Zulaessigkeittskontrolle** *P*olicy *E*nforcement *P*oint
   - durch Objektverwalter oder OS
   - beim Objektzugriff: Chekc der Gueltigkeit der Bescheinigung
   - kein Zugriff auf die Rechteinformation, z.B. ACL, notwendig
**Hybride Konzepte**
   - PDP auf basis von ACLs  
   - PEP bei Vorlage von Capabilities

# Kapitel 9: Systemsicherheit und Secure Programming
## 9.1 Architektur und Aufgabe eines OS
   - Hardware Abstraktion: Syscalls
   - Steuerung und Kontrolle:
     - Speicher-, CPU-, Dateiverwaltung, E/A
   - Modi: (Schutz)
     - Kernel-Mode: priv. Befehle, Betriebssystem laueft im Kernel-Mode
     - User-Mode: nicht-privilegierte Befehle, Nutzerprogramme

## 9.2 Schutz- und Kontrollaufgaben des Betriebssystems:
  - Identifikatoren und deren sichere Verwaltung
  - Zugangskontrolle
    - Authentisierung (Kap. 5)
  - Zugriffs- und Informationsflusskontrolle
    - Konzepte: ACL, Capailities, Labeling/Klassifizierung
    - Kontrollen
      - pruefen der ACLs/Capabilities/Token/Handles
      - pruefen der zulaessigen Fluesse gemaess z.B. BLP Regeln
  - Sicheres Hochfahren: Secure Boot, Trusted Boot
  - Arbeitsspeicherschutz RAM: Canaries, DEP, ASLR
  - Virtualisierung: u.a. Isolation: Hypervisor, VM-Monitor: Xen, LVM
  - Festplattenverschluesselung: Bitlocker, dm-crypt/LUKS

## 9.3 *T*rusted *C*omputing *B*ase *TCB*
   - TBC, Sicherheitskern
     - Menge der Software- und Hardwarekomponenten und Kontrollen, die zusammenwirken, um die Sicherheitsdienste des Systems durchzusetzen
     - Die TBC fasst die sicherheitskritischen Funktionen der Hardware und des BS zusammen
  
**Anforderung an Systemarchitekturen:**
  - Die TBC sollte moeglichst klein sein
  - Umfasst einen Grossteil der Hardware, Teile des OS-Kerns: u.a. Prozesswechsel, Speicherverwaltung, E/A-Mgmt
  - In der Praxis: i.d.R. sehr grosse TCB bei monolithischen Systemen
  - Verbesserungsansaetze: modular, Microkerne, Unikernel

## 9.4 Speicherschutz
   - Von-Neumann
   - Prozessadressraum: Stack, Heap, Text(Code)
**Nutzen des Laufzeitkellers:**
   - Methodenaufruf: call a
     1. Stackbereich reservieren
     2. mit Werten beschreiben
     3. Ruecksprungadresse: befehl im Code-Segment, Ausfuehrung nach Return.
**Stack/Heap bereiche sind haeufige Angriffsziele!**
