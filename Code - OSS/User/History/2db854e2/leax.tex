...
The techniques discussed here are specifically designed to counteract man-in-the-middle attacks, to be precise signal cancellation and signal overshadowing attacks. In a signal cancellation attack, the attacker
attempts to eliminate a signal from the channel by transmitting its inverse, thereby neutralizing the original signal. Conversely, signal overshadowing refers to an attack where a more powerful signal is transmitted in such a manner that it drowns out the original signal, making it inaudible to the recipient.
\begin{itemize}
