...
Smart home applications, small medical implants and a broad spectrum of internet-of-things gadgets has made direct connections from devices with minimal resources increasingly prevalent. In an era where the amount of small electronic and interconnected devices is constantly increasing, secure connections between them are of utmost importance. These connections are typically secured through public-key infrastructure or pre-shared keys. However, when these are not available, achieving a secure connection can pose a challenge. While out-of-band (OOB) channels, such as visual signals or sound, can be used to ensure message integrity, not all devices are equipped with these capabilities or permit the use of such channels. This is where In-Band trust establishment comes into play. In this paper, we present five methods that can be used to protect a messages integrity without the use of pre-shared secrets or extensive human involvement and demonstrate how they can be used to strengthen existing integrity protection mechanisms. We begin by introducing the reader to the concepts of Manchester Code (MC) and ON/OFF-Keying (OOK), which are fundamental in this field. We then provide a brief description of the Diffie-Hellman Key Agreement, which is of crucial importance as it will be fortified by most of our presented methods. We continue by introducing signal cancellation and signal overshadowing as the most relevant man-in-the-middle attacks on message integrity during a trust initialization process. After a brief introduction to the background of the provided techniques, we commence with the presentation of Integrity Codes [1]. These codes are a form of encoding that guarantees the integrity of messages, assuming there is no signal anti-blocking. We then proceed with Tamper-Evident Pairing (TEP) [2], a protocol developed to be the first strictly in-band pairing method, based on push-button configuration. Another method for in-band trust initialization is HELP [3]. Despite the cost of using a helper device, this technique can detect manipulations, even if an attacker can completely cancel a signal on the used channel. The fourth method presented, Chorus [4], was inspired by TEP and Integrity Codes. Its key distinction is its scalability. The final technique introduced is VERSE [5], another scalable method for initializing trust through in-band communication. Each method is presented in detail, complete with examples of potential real-world applications. Our main contribution is an overview about different in-band trust initialization techniques and a brief discussion about them.

\section{Background}

\subsection{ON/OFF-Keying and Manchester Code}

The majority of methods outlined here employ ON/OFF-Keying (OOK) as a modulation technique for transmitting messages. OOK, the most basic form of Amplitude Shift Keying, signifies a ''1'' bit with the presence of a signal and a ''0'' bit with its absence. This modulation method is used in conjunction with Manchester Code (MC). In the context of Manchester Code, a ''1'' bit is represented as ''10'', and a ''0'' bit is represented as ''01'', or vice versa.

\subsection{Diffie-Hellman Key Agreement}

The Diffie-Hellman Key Agreement is a protocol designed to securely exchange
cryptographic keys over a public network. It works as follows:\\
Each participant creates a secret key and a corresponding public key. The public key is then transmitted over the public network. Each participant, using their own secret key and the other participant's public key, generates a shared secret key. This shared secret key is identical for both participants. However, the Diffie-Hellman Key Agreement is susceptible to man-in-the-middle attacks, where an attacker manipulates the exchanged keys in such a way that they decrypt future messages. To counter this, many of the techniques described here aim to implement a variant of this agreement that safeguards the key exchange from such attacks.

\subsection{Attack Models}

The techniques discussed here are specifically designed to counter man in the middle attacks, where an attacker attempts to alter a transmitted message by manipulating the signal used for its transmission. Our primary focus lies on signal cancellation and signal overshadowing attacks. In a signal cancellation attack, the attacker attempts to eliminate a signal from the channel by transmitting its inverse, thereby neutralizing the original signal. Conversely, signal overshadowing refers to an attack where a more powerful signal is transmitted in such a manner that it drowns out the original signal, making it inaudible to the recipient.
...
They can be used in settings, where a bit ''0'' can be changed into a bit ''1'' but the reverse transformation is not feasible. The impossibility of the removal of a ''1'' bits energy is called signal anti-blocking.
Cagalj et al provided the idea of integrity codes, where a recipient can ensure the integrity of a message based on the encoding.
I-Codes consist of a tripple ($ \mathcal{S}, \mathcal{C}, e$), where $\mathcal{S}$ is a finite set of possible source states, $\mathcal{C}$ resembles a set of codewords that is also finite and $e$ is a source encoding rule with three limitations.
...
\begin{figure}
    \includegraphics[width=0.489\textwidth]{img/ICodes.png}
    \caption{I-Coding}
\end{figure}
...
The reverse direction of bit flipping is possible, but since usualy either manchester codes or some type of code with fixed hamming weight is used, the attacker must erase at least one signal to preserve the message's validity. Any invalid message is simply dropped by the receiver.
Synchronization between sender and receiver is mandatory. It is achieved through the use of specially designed bit strings known as delimiters. The sender transmits a signal containing a sequence of the message bracketed by two consecutive delimiters. Simultaneously, the receiver is actively monitoring the channel for any transmitted messages. 
...
\begin{itemize}
    \item Kaum real-world beispiele. warum?
\end{itemize}

\section{Conclusion}
In this paper we gave a brief overview of five different techniques for in-band trust establishment. 
