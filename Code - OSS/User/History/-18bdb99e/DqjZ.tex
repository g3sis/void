...
\section{HELP}
HELP, a device pairing primitive that operates solely in-band, was developed by Ghose et al. They designed a physical layer primitive to verify the integrity of a message. The primitive operates as follows: The system comprises a base station BS, to which all legitimate devices must connect, and a Helper device, which already maintains an authenticated connection with the Base Station. A new device D wants to link up to the base station using a manchester encoded message sent over a channel using on-off keying. The helper device H is placed near D. Both devices have to be set into some kind of pairing mode by some type of short interaction. H then proceeds to send an \textit{authenticated-request to communicate} to the BS using an authenticated encryption function $AE(\cdot)$. Folowwing this initiation step, BS sends a publicly known SYNC frame to D and H which synchronizes the clocks of each device. D then transmits a pair of its hashed message $h(m_D)$ and the message $m_D$ to BS in the way previously described. Simultaneously H transmits a signal $m_h$ with a random number of On-Slots, which are determined by a vector $s$ and are time alligned to the ones of $H(m_D)$. This sequence is most likely not a valid manchester encoded signal. After BS receives both messages H reveals $AE(s,K)$ to BS. BS then decrypts and verifies the integrity of $s$ using a function $VD(\cdot)$, which is the corresponding decryption and verification function for $AE(\cdot)$. If this function failes, $m_D$ is aborted. If not, BS checks if all slots depicted in $s$ are On-Slots in the received message of D. \\
