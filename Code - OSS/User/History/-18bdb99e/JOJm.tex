...
During experimentation with these mechanics, Capkun et al. discovered that I-Codes offer sufficient robustness for the transmission of short messages. Additionally, they developed a new concept called ``Authentication through presence'' and outlined two exemplary use cases. This concept works by sending the message $m$ whose integrity needs to be proven over a channel $C_1$ and a message digest $h(m)$ over another channel $C_2$ using I-Coding, with $h(\dot)$ being a one way function used to protect the integrity of the  message. \\
