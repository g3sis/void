...
The Helper device (H) is placed near D\@. Both devices have to be set into some
kind of pairing mode by some type of short interaction. H then proceeds to send
an authenticated-request to communicate to the BS using an authenticated
encryption function AE(⋅)AE(⋅). Following this initiation step, BS sends a
publicly known SYNC frame to D and H which synchronizes the clocks of each
device.

D then transmits a pair of hashed message h(mD)h(mD) and the message mDmD to BS
in the way previously described. Simultaneously, H transmits a signal mhmh with
a random number of On-Slots, which are determined by a vector ss and are
time-aligned to the ones of H(mD)H(mD). This sequence is most likely not a valid
Manchester-encoded signal.

After BS receives both messages, H reveals AE(s,K)AE(s,K) to BS\@. BS then
decrypts and verifies the integrity of ss using a function VD(⋅)VD(⋅), which is
the corresponding decryption and verification function for AE(⋅)AE(⋅). If this
function fails, mDmD is aborted. If not, BS checks if all slots depicted in ss
are On-Slots in the received message of D, with any kind of alteration leading
to the abortion of the protocol.

Afterwards, the helper's message is removed from the received signal and the
integrity of the received h(mD)′h(mD)′ is checked. The BS or H display FAILURE
or SUCCESS on a failed/successful attempt.
