...
An advanced scheme of Chorus called \textit{FH-CHorus} uses uncoordinated frequency hopping (UFH) to defend against such an attacker. The key principle here is to minimize the probability of cancelation by changing frequency multiple times during each slot by splitting it into $m$ minislots of a uniform length. 
The nodes now hop among the set of available channels for each minislot during its on slots. The same goes for off slots, whith the brief difference of listening instead of transmitting random information on the channel. If noise is detected during $m$ of an OFF-Slot, abort is sent. If the channel is slient during the wohle OFF-Slot, the node accepts. 
The complication of a needed frequency rendevous is also simplified by UFH. The coordinator simply sends a synchronization package containing a counter over every channel while hopping onto $m$ frequencys and sending for a slot length of $T_s$ increasing the counter until it equals $m$. Every other node, initialy asynchrounously, hops the channel with a longer slot length $T_s'$. As soon as a synchronization package is detected, the counter is decoded and the node beginns listening after $(m-counter)*T_s +T_cts$ seconds   
