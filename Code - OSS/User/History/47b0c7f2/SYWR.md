...
- A full joint probability distribution can answer any question about a domain, but can become intractably large as the number of vars grows
...
### Conversion of Higher-Order Markov Chains
  ...
  > A Markov chain of mth order is defined as:  
  > P(Xn=xi|Xn-1=xj,Xn-2=xk,...,X1=xo)  
  > = P(Xn=xi|Xn-1=xj,Xn-2=xk,...,Xn-m=xl) for n > m  
...
### Sensor Model
  In many examples, the state of a system cannot ve directly measured and has to be infered from sensor values.  
...
### Hidden Markov Model
  Markov chain + Sensor Model = *hidden Markov model* (HMM)  
  - probabilities 
    - (p_i)_n = P(X_n = x_i)
    - (p^_i)_n = P(E_n = e_i)
  - matricies
    - T_i,j = P(X_n = x_i|X_n-1 = x_j)
    - H_i,j = P(E_n = e_i|X_n = x_j)
  - probabilites can be computed as 
    - p_n = Tp_(n-1)
    - p^_n = Hp_n
